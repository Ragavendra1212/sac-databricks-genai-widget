(function () {
  console.log("RaghavDatabricksHardcoded.js: script loaded");
  ...
  template.innerHTML = `
    ...
    <div class="wrapper">
      <div class="header">
        <div class="title" id="title">Databricks GENAI (Hardcoded PAT)</div>
  ...
  class RaghavDatabricksHardcoded extends HTMLElement {
    constructor() {
      super();
      this._props = {};

      this.title = "Databricks GENAI (Hardcoded PAT)";
    ...
    onCustomWidgetAfterUpdate(changedProperties) {
      if ("title" in this._props) {
        this.title = this._props.title || "Databricks GENAI (Hardcoded PAT)";
        if (this._titleEl) {
    ...
    _callDatabricks() {
      ...
      if (!endpoint) {
        this._outputEl.textContent =
          "Endpoint URL is not set. Please configure 'endpointUrl' in properties or update DATABRICKS_ENDPOINT_DEFAULT.";
        return;
      }

      if (!token || token.startsWith("<PUT_")) {
        this._outputEl.textContent =
          "HARD-CODED PAT IS NOT SET in RaghavDatabricksHardcoded.js. Please set DATABRICKS_PAT.";
        return;
      }
  ...
  customElements.define(
    "com-raghavendra-sap-databrickshcwidget",
    RaghavDatabricksHardcoded
  );
})();
